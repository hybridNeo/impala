...
void QueryState::InitFilterRoutingTable(
  ...
  for (auto& it : filter_routing_table) {
    std::pair<int32_t, FilterState> table_entry =
        make_pair(it.first, impala::FilterState::FromThrift(it.second));
    filter_routing_table_.insert(table_entry);
  }
}

TNetworkAddress QueryState::GetAggregatorAddress(int32_t filter_id) {
  auto it = filter_routing_table_.find(filter_id);
...
void QueryState::UpdateFilter(const TUpdateFilterParams& params) {
  ...
  // TODO: Call PublishFilterFromAggregator() asynchronously from threadpool.
  ...
  for (auto backend : backend_list_) {
    for (int fragment_idx : target_fragment_idxs) {
      if (backend.second.count(fragment_idx) == 0) continue;
      ...
      LOG(INFO) << "DebuggingPublishFilter Aggregator sending filter to fragment with id "
                << fragment_idx;
      PublishFilterFromAggregator(rpc_params, backend.first);
    }
  }
}
