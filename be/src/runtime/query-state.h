...
class QueryState {
 public:
  ...
  void InitFilterRoutingTable(const std::map<int32_t, TFilterState>& filter_routing_table);
 ...
 private:
  ...
  /// QueryState's copy of the filter routing table, contains aggregator information.
  std::map<int32_t, FilterState> filter_routing_table_;

  /// lock used in UpdateFilter, guards filter_routing_table_.
  SpinLock filter_lock_;

  /// holds information regarding fragments executing in a backend(TNetworkAddress)
